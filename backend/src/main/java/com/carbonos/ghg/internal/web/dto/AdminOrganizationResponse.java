 ...
 */
public record AdminOrganizationResponse(UUID id, String name, long accountNo, List<String> ownerEmails,
		long memberCount, SupportAccessResponse supportAccess) {
	...
	public static AdminOrganizationResponse from(SupportAccessService.OrganizationSummary summary) {
		return new AdminOrganizationResponse(summary.organization().getId(), summary.organization().getName(),
				summary.organization().getAccountNo(), summary.ownerEmails(), summary.memberCount(),
