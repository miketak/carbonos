...
| 1a | As the Newcomer (who entered it), click **Approve** on the row. | Refused: "You entered 'Emulsion explosive (supplier)'. A factor is checked by someone other than the person who typed it (Corporate Standard chapter 7): ask <the Analyst> to approve it." Approval is a control (spec 02.11), and the organization has another member who may write. | | |
| 1b | As the Analyst, click **Approve** on the row, then read it. | The badge reads **Approved** with "by <analyst email> on <date>" under it. Unapprove and approve again to see the record follow the act. Leave it **unapproved** afterwards: procedure 5 case B7 shows the gate refusing it until it is approved. | | |
| 1c | In an organization where you are the only member who may write (create a scratch one), add a factor and approve it yourself. | The approval goes through and the row reads "self-approved: nobody else could check it". The run report prints the same under its factor table (spec 02.11). Delete the scratch organization afterwards. | | |
