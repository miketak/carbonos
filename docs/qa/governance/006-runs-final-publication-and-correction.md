...
one (0.84 kg/litre, 3,571.4286 litre) flagged as a proxy.
...
| 4 | Read the by-gas table. | The refrigerant line carries 20 kg under HFCs. The flights factor publishes CO₂e only, so its 3,900 kg sits on the row "CO₂e from factors without a gas split". The footing row "Total (scope 2 location-based), ties to section 04" equals the section 04 total. | | |
...
| Step | Action | Expected result | Pass/Fail | Notes |
| --- | --- | --- | --- | --- |
| 1 | On **Report**, fill in **Approved by** "Kofi Mensah, Reviewer", the uncertainty statement "Metered fuel and electricity; the flights figure is the travel agent's estimate", and add the denominator "Product output", value 1500, unit `t`. Save. | Saved. | | |
| 2 | Reload the page. | Every field reads what you typed. The denominator is on the inventory, not in the browser. | | |
| 3 | Launch Run 005 and open its report. | The header names the approver. **Intensity** reads "0.080249 t CO₂e per t of product output (1,500 t)", 120.373 t divided by 1,500. The methodology section prints the uncertainty statement and ends "2 of 10 lines record a quantitative uncertainty" with the emissions-weighted figure. | | |
...
| 3 | Open the PDF. | The methodology section prints "Approved by the person who entered them, no other member of the organization being able to check them at the time: Diesel (Solo).". Adansi's PDF of case D1 prints no such sentence: Kofi checked its factors. | | |
...
| Step | Action | Expected result | Pass/Fail | Notes |
| --- | --- | --- | --- | --- |
| 1 | As Esi in the private window, open FY2025 and click **Mark as final** on Run 005. | The confirm dialog ends in "This action needs the REVIEWER or OWNER role in the organization.". | | |
...
| 5 | As Esi, click **Publish**. | Refused with the same role sentence as step 1. | | |
...
| Step | Action | Expected result | Pass/Fail | Notes |
| --- | --- | --- | --- | --- |
| 1 | Click **Create correction** and give the reason "typo". | Refused: "A correction needs a reason of at least 10 characters: what was wrong in the published inventory.". | | |
